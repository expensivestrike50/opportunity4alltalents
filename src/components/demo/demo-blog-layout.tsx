...
interface DemoBlogLayoutProps {
  children: React.ReactNode;
  breadcrumbs?: Crumb[];
}

const menuItems = [
  { title: "All Posts", url: "/demo/dashboard/blog", icon: FileText },
  { title: "Create Post", url: "/demo/dashboard/blog/new", icon: PenSquare },
];

const DemoBlogLayout = ({ children, breadcrumbs }: DemoBlogLayoutProps) => {
  const location = useLocation();
  const { profile } = useSeedData();
  const crumbs: Crumb[] = breadcrumbs?.length
    ? breadcrumbs
      ...
      <div className="min-h-screen flex w-full bg-black">
        <Sidebar collapsible="icon">
          <SidebarHeader>
            <Link to="/" className="flex items-center gap-2 px-2">
              <BrandLogo markClassName="h-6 w-7" textClassName="text-xs group-data-[collapsible=icon]:hidden" />
            </Link>
          </SidebarHeader>
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  <SidebarMenuItem>
                    <SidebarMenuButton
                      asChild
                      isActive={location.pathname === "/demo/dashboard/profile"}
                    >
                      <Link to="/demo/dashboard/profile">
        ...
        <main className="flex-1 flex flex-col min-h-screen overflow-hidden">
          <header className="flex items-center gap-3 sm:gap-4 px-4 py-3 sm:px-6 border-b border-white/10 bg-black/50 backdrop-blur-sm sticky top-0 z-10">
            ...
            <Badge variant="color" className="ml-auto shrink-0">
              Demo data
            </Badge>
          </header>
