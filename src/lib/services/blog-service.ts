...
import { demoBlogPosts } from "@/demo/seed-data";
...
/**
 * Falls back to bundled seed content when Supabase has no backing table/rows
 * (e.g. no real project connected yet) so the public site isn't empty.
 */
const demoFallback = (category?: string, includeDrafts = false): BlogPost[] => {
  let posts = includeDrafts ? demoBlogPosts : demoBlogPosts.filter((post) => post.status === PUBLISHED);
...
export const getBlogPosts = async (
  ...
  if (error) {
    ...
    return demoFallback(category, includeDrafts);
  }
...
export const getBlogPostBySlug = async (slug: string): Promise<BlogPost | null> => {
  ...
  return demoBlogPosts.find((post) => post.slug === slug && post.status === PUBLISHED) ?? null;
};
...
export const getBlogPostById = async (id: string): Promise<BlogPost | null> => {
  ...
  return demoBlogPosts.find((post) => post.id === id) ?? null;
};
...
export const getCategories = async (includeDrafts = false): Promise<string[]> => {
  ...
  if (!data || data.length === 0) {
    const categories = demoFallback(undefined, includeDrafts).map((post) => post.category);
